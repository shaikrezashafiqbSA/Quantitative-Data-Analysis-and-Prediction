...
from order_managers.model_state_default import model_state_default
from settings import KGI_URL,KGI_BEARER



BASE_CURRENCY = os.getenv("BASE_CURRENCY")    
# set base currency to USD if not set
if BASE_CURRENCY is None:
    BASE_CURRENCY = "USD"
    warnings.warn("BASE_CURRENCY not set, defaulting to USD")
...
class KGI_Client():
    ...
    def __init__(self,model_config):
        self.url = KGI_URL
        self.auth_token = KGI_BEARER
        self.headers = {'Authorization': 'Bearer '+ self.auth_token}
        self.model_config = model_config
        
    # get BASE_CURRENCY from setting


        
    def create_market_order(self, symbol:str, side:str, quantity:float) -> dict:
        if side == "buy":
            #TODO symbol has to be symbol_from and symbol_to
    ...
    def cancel_order_model_state_update(self, model_state, position, side):
        if position == "L" and side == "buy":
            side_to_order = "buy"
            side_to_order_model_state = "entry"
        elif position == "L" and side == "sell":
            side = "sell"
            side_to_order_model_state = "exit"
        elif position == "S" and side == "sell":
            side = "sell"
            side_to_order_model_state = "entry"
        elif position == "S" and side == "buy":
            side = "buy"
            side_to_order_model_state = "exit"
        else:
            raise Exception(f"Invalid position and side combination: {position} - {side}")
        if model_state[position][f"{side_to_order_model_state}_pending"]:
            quantity_to_cancel = model_state[position][f"{side_to_order_model_state}_quantity_expected"]
            ...
            cancel_order_resp, cancel_timestamp = self.cancel_order(symbol=self.model_config["instrument_to_trade"], 
                                                                    side = side_to_order,
                                                                    quantity=quantity_to_cancel,
                                                                    order_id = order_id_to_cancel)
            # THIS ASSUMES order can has been immediately cancelled
            model_state[position][f"{side_to_order_model_state}_pending"] = False
            model_state[position][f"{side_to_order_model_state}_quantity_expected"] = 0
            model_state[position][f"{side_to_order_model_state}_price_expected"] = 0
    ...
    def send_orders(self, model_state):
        ...
        """
        symbol = self.model_config["instrument_to_trade"]
        ...
        if model_state["L"]["entry_now"]:
            ...
            model_state = self.cancel_order_model_state_update(model_state, position=pos, side = "buy")
            model_state = self.cancel_order_model_state_update(model_state, position="S", side = "sell")
            
            price_to_record = model_state[pos]["entry_price_expected"]
            price_to_order = price_to_record
            quantity_to_order = model_state[pos]["entry_quantity_expected"] 
        # ===================================
        # EXIT     
        # ===================================
        elif model_state["L"]["exit_now"]:
            position = "long"
            pos = "L"
            # Prepare order params
            side_to_order= "sell"
            ...
            model_state = self.cancel_order_model_state_update(model_state, position=pos, side = "sell")
            
            price_to_record = model_state[pos]["exit_price_expected"]
            price_to_order = 1/price_to_record
            quantity_to_order = model_state[pos]["entry_quantity"] 
        ...
        # ===================================
            ...
            in_position_flag = True
            # CANCEL ALL PENDING ENTRY ORDERS (if any) FIRST
            model_state = self.cancel_order_model_state_update(model_state, position=pos, side = side_to_order)
            model_state = self.cancel_order_model_state_update(model_state, position="L", side = "buy")
        ...
        if self.model_config["run_mode"] == "live" and self.model_config["order_type"] == "market":
            ...
            if (order_resp["execution_status"] == "F").any():
                ...
                model_state[pos]["signal_event"] = False
                model_state[pos]["in_position"] = True
                model_state[pos][f"{side}_pending"] = False
                model_state[pos]["order_id"] = order_resp["cl_order_id"].iloc[0]
                model_state[pos]["entry_now"] = False
                model_state[pos]["exit_now"] = False
                model_state[pos][f"{side}_price"] = float(order_resp["average_price"].iloc[-1])
                model_state[pos][f"{side}_quantity"] = float(order_resp["quantity"].iloc[-1])
                # return model_state
            else:
                logger.info("EXECUTION STATUS: PENDING") 
                model_state[pos]["signal_event"] = False
                model_state[pos]["in_position"] = False
                model_state[pos][f"{side}_pending"] = True
                model_state[pos]["order_id"] = order_resp["cl_order_id"].iloc[0]
                model_state[pos]["entry_now"] = False
                model_state[pos]["exit_now"] = False
                model_state[pos][f"{side}_price"] = 0
                model_state[pos][f"{side}_quantity"] = 0
                # return model_state
            ...
            # ============================================================================= 
            model_state = self.save_to_db(self.model_config, order_resp, model_state, position=pos, side=side_to_order)
        ...
        # =============================================================================   
            ...
            logger.info(f"\n{'='*30}\nSENDING ORDER for {pos} | {side_to_order}\n{'='*30}\n\nexpected price: {price_to_order}\nexpected quantity: {quantity_to_order}\n\n")
            ...
            if (order_resp["execution_status"] == "F").any():
                ...
                self.save_to_db(self.model_config, order_resp, model_state, position=pos, side=side_to_order)
                
                # Update model_id for next trade if close position, else dont update
                model_state[pos]["model_id"] = model_state[position]["model_id"]+1 if in_position_flag == False else model_state[position]["model_id"]
                model_state[pos]["signal_event"] = False
                model_state[pos]["in_position"] = in_position_flag
                model_state[pos]["entry_now"] = False
                model_state[pos]["exit_now"] = False
                model_state[pos][f"{side_to_order}_pending"] = False # <------------------------------- Pending order set to False since filled immediately
                model_state[pos]["order_id"] = order_resp["cl_order_id"]
                model_state[pos][f"{side_to_order}_price"] = float(order_resp["average_price"].iloc[-1])
                model_state[pos][f"{side_to_order}_quantity"] = float(order_resp["quantity"].iloc[-1])
                ...
                model_state[pos]["signal_event"] = False
                model_state[pos]["buy_signal"] = False
                model_state[pos]["sell_signal"] = False
                model_state[pos][f"{side_to_order}_pending"] = True  # <------------------------------- Pending order set to True
                model_state[pos]["order_id"] = order_resp["cl_order_id"].iloc[-1]
                model_state[pos][f"{side_to_order}_price_expected"] = price_to_record
                model_state[pos][f"{side_to_order}_quantity_expected"] = quantity_to_order
        ...
        # ============================================================================= 
            ...
            order_resp = self.paper_order_resp(quantity_to_order, side, model_state)
            
            model_state[pos]["in_position"] = True
            model_state[pos]["entry_now"] = False
            model_state[pos]["exit_now"] = False
            model_state[pos][f"{side}_price"] = order_resp["average_price"]
            model_state[pos][f"{side}_quantity"] = order_resp["quantity"]
...
# ==========================================================================================================================================================
    ...
    def load_model_state(self, model_config=None, transactions_df=None, model_state=None) -> dict:
        ...
        for position in ["L", "S"]:
            ...
            if len(t_df_i) == 0:
                ...
                last_id = t_df_i["model_id"].iloc[-1]
                last_side =  t_df_i["side"].iloc[-1]
                
                if last_side == "buy":
                    last_side_model_state = "entry"
                elif last_side == "sell":
                    last_side_model_state = "exit"
                
                if position == "long":
                    position_model_state = "L"
                elif position == "short":
                    position_model_state = "S"
                model_state[position_model_state]["model_id"] = last_id
                model_state[position_model_state]["in_position"] = True
                model_state[position_model_state][f"{last_side_model_state}_quantity"] = t_df_i["quantity"].iloc[-1]
                model_state[position_model_state][f"{last_side_model_state}_price"] = t_df_i["average_price"].iloc[-1]
                model_state[position_model_state][f"{last_side_model_state}_price_expected"] = t_df_i["expected_price"].iloc[-1]
            else:
                """
                ...
                last_t_df_i = t_df_i[t_df_i["model_id"]==last_id]
                if position == "L":
                ...
                if buy_sell_diff > 0:
                    model_state[position_model_state]["model_id"] = last_id
                    model_state[position_model_state]["in_position"] = True
                    model_state[position_model_state][f"{last_side_model_state}_quantity"] = buy_sell_diff
                    model_state[position_model_state][f"{last_side_model_state}_price"] = t_df_i["average_price"].iloc[-1]
                    model_state[position_model_state][f"{last_side_model_state}_price_expected"] = t_df_i["expected_price"].iloc[-1]
                elif buy_sell_diff == 0: 
                    # NO OPEN POSITION! Update only model_id 
                    model_state[position_model_state]["model_id"] = last_id+1
                else:
    ...
    def update_model_state(self, model_state, model_config, replay_dt = None):
        """
        ...
        for position in ["L","S"]:
            ...
            for side in ["entry","exit"]:
                if model_state[position][f"{side}_pending"] and (order_executions_df_by_id["execution_status"] == "F").any():
                    ...
                    self.save_to_db(model_config, order_resp, model_state, position=position, side=side_to_record)
                    # ORDER FILL on long buy or short sell --> position has been OPENED
                    if (side == "entry" and position=="long") or (side == "exit" and position=="short"):
                        model_state[position]["in_position"] = True
                    # ORDER FILL on long sell or short buy --> position has been CLOSED
                    elif (side == "exit" and position=="long") or (side == "entry" and position=="short"):
                        model_state[position]["in_position"] = False
                        model_state[position]["model_id"]+=1
                    
                    model_state[position][f"{side}_price"] = float(filled_order_executions_df_by_id["average_price"].iloc[0])
                    model_state[position][f"{side}_quantity"] = float(filled_order_executions_df_by_id["quantity"].iloc[0])
    ...
    def save_to_db(self, model_config, order_resp, model_state, position, side):
        ...
        metric_dict_to_insert = {"order_id":model_state[position]["model_id"],
                                 ...
                                 "account": model_config["account"],
                                 "model_name":model_config["model_name"],
                                 "model_id": float(model_state[position]["model_id"]),
                                 "symbol": model_config["instrument_to_trade"],
    ...
    def consolidate_pnl(self,model_config, transactions_df = None):        
        ...
        for position in ["long", "short"]:
            if position == "long":
                ...
                position_model_state = "S"
            try:
                L_orders0 = transactions_df[transactions_df[f"{position_model_state}_positions"]==position].copy()
                logger.info(f"\n\nConsolidating pnl for {position} | {len(L_orders0)} orders\n: {L_orders0}\n\n")
            except Exception as e:
                logger.error(f"Error in consolidating pnl for {position} - {e}\ntransactions_df: {transactions_df}")
                raise Exception(e)
            L_orders = pd.DataFrame()

            if L_orders0 is None or L_orders0.empty:
                L_orders["created_at"] = np.nan
                L_orders["open_at"] = np.nan
                L_orders["close_at"] = np.nan
                L_orders["model_name"] = np.nan
                L_orders["account"] = np.nan
                L_orders["symbol"] = np.nan
                L_orders["instrument_type"] = np.nan
                L_orders["reporting_currency"] = np.nan
                L_orders["position"] = np.nan
                L_orders["model_id"] = np.nan
                L_orders["positions"] = np.nan
                L_orders["actual_entry_price"] = np.nan
                L_orders["expected_entry_price"] = np.nan
                L_orders["cost"] = np.nan
                L_orders["actual_qty"] = np.nan
                L_orders["expected_qty"] = np.nan
                L_orders["actual_exit_price"] = np.nan
                L_orders["expected_exit_price"] = np.nan
                L_orders["actual_fees"] = np.nan
                L_orders["expected_fees"] = np.nan
                L_orders["actual_pnl_pct"] = np.nan
                L_orders["expected_pnl_pct"] = np.nan
                L_orders["actual_rpnl"] = np.nan
                L_orders["expected_rpnl"] = np.nan
            else:
                ...
                L_orders["position"] = position_model_state
                L_orders["model_id"] = L_orders0["model_id"]
                ...
                L_orders["expected_fees"] = model_config["fee"][0]
            ...
            for L_id in np.unique(L_orders["model_id"]):
                L_orders_i = L_orders[L_orders["model_id"]==L_id]
