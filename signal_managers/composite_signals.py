...
def merge_sig_dicts(sig_dicts, 
                    instrument_to_trade = "ccxt_kucoin__BTC-USDT",
                    timeframe_to_trade = "1h",
                    reduce_col_names = True):
    """
    This function merges all the instruments_dict into one big dataframe.

    Args:
    sig_dicts: The dictionary of the signal dictionaries.

    Returns:
    A dataframe with all the signals.
    """
    # Find the df with instrument, timeframe to trade.
    df_to_trade = None
    for instrument, sig_dict in sig_dicts.items():
        for timeframe, df in sig_dict.items():
            if instrument == instrument_to_trade and timeframe == timeframe_to_trade:
                df_to_trade = df.drop(columns=["date_time"]).copy()
                # print(df_to_trade)
    if df_to_trade is None:
        raise Exception(f"Could not find {instrument_to_trade} {timeframe_to_trade} in sig_dicts.")
    
    # Merge all the dfs into one big df on df_to_trade
    for instrument, sig_dict in sig_dicts.items():
        for timeframe, df in sig_dict.items():
            if instrument == instrument_to_trade and timeframe == timeframe_to_trade:
                continue
            if reduce_col_names:
                df = df.drop(columns=["date_time"]).copy().add_suffix(f"_{instrument.split('__')[-1]}_{timeframe}")
            else:
                df = df.drop(columns=["date_time"]).copy().add_suffix(f"_{instrument}_{timeframe}")
            # print(list(df_to_trade.tail(3).index))
            df_to_trade = pd.merge(df_to_trade, df, how="left", right_index=True, left_index=True)
            df_to_trade.fillna(method="ffill", inplace=True)
            df_to_trade.dropna(inplace=True)
            # print(f"----> {list(df_to_trade.tail(3).index)}")
...
# def merge_signals(sig_dicts, smallest_timeframe="5Min", target_signals=["comp_sig"]):
#     for instrument, sig_dict in sig_dicts.items():
#         df= sig_dicts[smallest_timeframe]
