...
def two_layer_optimization(patience = 5,
    ...
    continuous_variables, non_continuous_variables, continuous_initial_values_dict, learning_rates, epsilons, clippings = organize_params(data_params_payloads)
...
def gradient_descent(data_params_payloads_i,
    ...
    epsilon_growth_factor = float(os.getenv('epsilon_growth_factor'))
    lr_schedule = int(os.getenv('lr_schedule'))
    lr_schedule_factor = float(os.getenv('lr_schedule_factor'))
    
    improvement_threshold = float(os.getenv('lr_adaptive_improvement_threshold'))
    lr_adaptive_increase_factor = float(os.getenv('lr_adaptive_increase_factor'))
    lr_adaptive_decrease_factor = float(os.getenv('lr_adaptive_decrease_factor'))
    # lr_decay_factor_str = os.getenv('lr_decay_factor')
    # lr_decay_factor = int(lr_decay_factor_str)

    original_epsilons = deepcopy(epsilons)
    original_learning_rates = deepcopy(learning_rates)  # Save the original learning rates
    ...
    for i in tqdm(range(gradient_descent_max_iterations ), desc = f"Gradient descent iterations i/n"):
        ...
        for param_name in continuous_values_dict_descent_updated.keys():
            ...
            if not global_minimum:
                ...
                updated_param_value = max(clippings[param_name][0], updated_param_value)
                updated_param_value = min(clippings[param_name][1], updated_param_value)
            # Global minimum
            else:
        ...
        if cost - best_cost > metric_min_delta:
            ...
            learning_rates = deepcopy(original_learning_rates)
        else:
            no_improvement_counter += 1
            # If there's no improvement, increase the epsilon values
            for param in epsilons.keys():
                epsilons[param] *= (1 + (epsilon_growth_factor * no_improvement_counter))
            logger.warning(f"\n\nNo significant improvement for {no_improvement_counter} counts. due to cost ({cost}) - best_cost ({best_cost}) < {metric_min_delta} \n\n")

        if lr_adaptive_strategy:
            # Learning rate decay adaptive: increase the learning rate by factor % if there's improvement else decrease by factor %?
            if cost - best_cost > improvement_threshold:
                lr_adaptive_factor = lr_adaptive_increase_factor
                for param in learning_rates.keys():
                    learning_rates[param] *= lr_adaptive_factor
            else:
                lr_adaptive_factor = lr_adaptive_decrease_factor
                for param in learning_rates.keys():
                    learning_rates[param] *= lr_adaptive_factor
            logger.info(f"\n\nScaled learning rates by {lr_adaptive_factor} at iteration {i}: \n{learning_rates}\n\n")

        else:
            # Learning rate decay schedule: reduce the learning rate by 50% every 100 epochs
            if i % lr_schedule == 0 and i != 0:
                for param in learning_rates.keys():
                    learning_rates[param] *= lr_schedule_factor
                logger.info(f"\n\nScaled learning rates by {lr_schedule_factor} at iteration {i}: \n{learning_rates}\n\n")
